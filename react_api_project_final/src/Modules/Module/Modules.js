  ...
  const searchModuleTitleHandler = (event) => {
    ...
    for (var i = 0; i < project_data.length; i++) {
      for (var j = 0; j < project_data[i].modules.length; j++) {
        if (
          project_data[i].modules[j].moduleTitle
            .toUpperCase()
            .includes(searchModuleTitle.toUpperCase())
        ) {
          setSearchResult(
            <tr>
              <Td
              ...
              <Td
                data={`${JSON.stringify(
                  project_data[i].modules[j].moduleDesc
                )}`}
              ></Td>
            </tr>
          );
          console.log(
            `${JSON.stringify(project_data[i].modules[j].moduleTitle)}`
          );
        }
      }
    }
  ...
  const deleteModuleHandler = (mid, pid) => {
    ...
    for (var i = 0; i < project_data.length; i++) {
      if (project_data[i].pid === pid) {
        project_data[i].modules = removeByAttr(
          project_data[i].modules,
          "mid",
          mid
        );
      }

...
import { addApi, deleteApi, editAPi, getApiData, searchApi, sort } from "../../Shared/Services/Api-Services";
...
const Api = () => {
  ...
  const submitApiHandler = async (apiProjectId, apiModuleId, apiTitle, apiDesc, apiUrl, apiMethod, apiRequest, apiResponse) =>{
    if(apiUrl !== "" && apiTitle !== "" && apiDesc !== "" && apiProjectId !== "" && apiModuleId !== "" && apiMethod !== "" && apiRequest !== "" && apiResponse !== "")
    {
      setButtonPopup(false);
      const data = await addApi(apiProjectId, apiModuleId, apiTitle, apiDesc, apiUrl, apiMethod, apiRequest, apiResponse);
      loadApiData();
    }
    else{
  ...
  const updateApiHandler = async(apiId,projId,moduleId,title,desc,apiUrl,apiMethod,apiRequest,apiResponse) =>{
    setButtonPopup(false);
    const data = await editAPi(apiId,projId,moduleId,title,desc,apiUrl,apiMethod,apiRequest,apiResponse);
    loadApiData();
  }
